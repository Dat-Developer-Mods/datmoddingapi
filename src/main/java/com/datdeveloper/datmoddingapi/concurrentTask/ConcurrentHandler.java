 ...
 * <p>For executing tasks with a delay but on the server thread (with everything else, nice and thread safely), use
 * {@link com.datdeveloper.datmoddingapi.delayedEvents.DelayedEventsHandler}</p>
 *
 * @see Callable
 * @see Runnable
 */
...
public class ConcurrentHandler {
    private static final String NOT_READY_WARNING = "Something attempted to schedule a task before the"
            + " ConcurrentHandler was ready, ignoring";
    ...
    public static void initialise() {
        INSTANCE.service = (ScheduledThreadPoolExecutor) Executors.newScheduledThreadPool(
                Math.clamp(DatConfig.getMaxConcurrentThreadCount() / 4, 1, 5)
        );
    ...
    /**
     * A Server tick event used to handle any Runnables that have been passed from a Concurrent thread to be executed on
     * the main thread
     *
     ...
     */
    public static <T> ScheduledFuture<T> scheduleConcurrentTask(final long delay,
                                                                final TimeUnit unit,
     ...
     * @return A ScheduledFuture representing the task
     */
    public static ScheduledFuture<?> scheduleConcurrentTask(final long delay,
                                                            final TimeUnit unit,
                                                            final Runnable task) {
        if (!ConcurrentHandler.INSTANCE.initialised) {
            throw new IllegalStateException(NOT_READY_WARNING);
     ...
     */
    public static ScheduledFuture<?> scheduleFixedRateConcurrentTask(final long initialDelay,
                                                                     final long period,
                                                                     final TimeUnit unit,
                                                                     final Runnable task) {
        if (!ConcurrentHandler.INSTANCE.initialised) {
